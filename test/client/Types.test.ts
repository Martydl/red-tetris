...
import { PieceGrid, PlayerStatus, Coords, Piece, Opponent, Room } from "../../client/src/Types";
import React from "react";

describe("Types", () => {
    const testPieceGridType: PieceGrid = [[]];
    const testCoordInterface: Coords = {x: 5, y: 10}
    const testPieceInterface: Piece = {name: 1, rotation: 2, pos: testCoordInterface}
    const testOpponentInterface: Opponent = {playerName: "playerTest", shadow: new Array(10).fill(0), status: PlayerStatus.ALIVE}
    const testRoomInterface: Room = {name: "roomTest", nbPlayers: 1, started: false}
